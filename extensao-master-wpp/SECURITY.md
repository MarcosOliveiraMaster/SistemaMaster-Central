...
| Vazamento de dados de mensagens (conteúdo sensível de aulas/clientes) | Nada é persistido em `chrome.storage` nem em log. A fila de envio existe só em memória (RAM) durante o processamento e é descartada ao concluir. A imagem (quando enviada) chega como data URL dentro do próprio payload — não é lida do clipboard nem de nenhuma fonte fora da chamada em curso. |
...
- **A imagem**: chega como data URL dentro do payload da chamada em curso — a extensão
  não lê o clipboard nem qualquer outra fonte do sistema.
