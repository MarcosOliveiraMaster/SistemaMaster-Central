        ...
        try {
          ...
          if (rows.length > 0) {
            ...
            for (const row of rows) {
              ...
              // Resolver clienteUid pelo CPF quando estiver ausente no documento
              let resolvedClienteUid = docData.clienteUid || docData.clientUid || '';
              if (!resolvedClienteUid) {
                const cpfDoCliente = docData.cpf || aula.cpf || '';
                if (cpfDoCliente) {
                  try {
                    const clienteSnap = await db.collection('cadastroClientes')
                      .where('cpf', '==', cpfDoCliente)
                      .limit(1)
                      .get();
                    if (!clienteSnap.empty) {
                      resolvedClienteUid = clienteSnap.docs[0].data().uid || '';
                    }
                  } catch (eUid) {
                    console.warn('[SalvarAulas] Não foi possível resolver clienteUid pelo CPF:', eUid);
                  }
                }
              }
